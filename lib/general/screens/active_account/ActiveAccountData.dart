...
class ActiveAccountData {
  ...
  void onActiveAccount(BuildContext context, String userId) async {
    if (formKey.currentState!.validate()) {
      ...
      var result = await GeneralRepository(context).sendCode(
          int.parse(code.text),userId);
      if(result){
  ...
  void onResendCode(BuildContext context, String userId) async {
   bool result =  await GeneralRepository(context).resendCode(userId);
   if(result){
     LoadingDialog.showSnackBar(context, tr(context,"codeSendSuccessfully"));
   }
